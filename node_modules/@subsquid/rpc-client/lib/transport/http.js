...
class RpcHttpClient extends http_client_1.HttpClient {
    constructor() {
        super(...arguments);
        this.fixUnsafeIntegers = false;
    }
    async handleResponseBody(req, res) {
        if (!res.ok)
            return super.handleResponseBody(req, res);
        let json = await res.text();
        try {
            if (this.fixUnsafeIntegers) {
                json = (0, util_internal_json_fix_unsafe_integers_1.fixUnsafeIntegers)(json);
            }
            return JSON.parse(json);
        }
        catch (err) {
            throw new errors_1.RpcProtocolError(1008, `server returned invalid JSON: ${err.message}`);
        }
    }
}
