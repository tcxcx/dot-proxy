...
exports.getRuntimeDescription = void 0;
...
const typesBundle_1 = require("./old/typesBundle");
const util_1 = require("./util");
function getRuntimeDescription(metadata, specName, specVersion, typesBundle) {
    if (typeof metadata == 'string' || metadata instanceof Uint8Array) {
        metadata = (0, codec_1.decodeMetadata)(metadata);
    }
    let oldTypes;
    if ((0, util_1.isPreV14)(metadata)) {
        (0, assert_1.default)(specName != null && specVersion != null, 'spec name and version are required for pre-V14 metadata');
        typesBundle = (0, util_internal_1.assertNotNull)(typesBundle || (0, io_1.getOldTypesBundle)(specName), `types bundle is required for ${specName} chain`);
        oldTypes = (0, typesBundle_1.getTypesFromBundle)(typesBundle, specVersion, specName);
    }
...
exports.getRuntimeDescription = getRuntimeDescription;
class FromV14 {
    ...
    signature() {
        ...
        let signedExtensionsType = {
            ...
            fields: this.metadata.extrinsic.signedExtensions.map(ext => {
                return {
                    name: (0, util_naming_1.toCamelCase)(ext.identifier),
                    type: ext.type
                };
            }).filter(f => {
                return !(0, util_1.isUnitType)(types[f.type]);
            }),
    ...
    uncheckedExtrinsic() {
        ...
        for (let i = 0; i < this.metadata.lookup.types.length; i++) {
            ...
            if (def.path[0] == 'avail_core' && (0, util_internal_1.last)(def.path) == 'AppUncheckedExtrinsic') {
                return i;
            }
        }
    ...
    getStorageItem(pallet, name) {
        let storage = this.storage();
        let item = storage[pallet]?.items[name];
        return (0, util_internal_1.assertNotNull)(item, `Can't find ${pallet}.${name} storage item`);
    }
    storage() {
        let storage = {};
...
//# sourceMappingURL=runtime-description.js.map