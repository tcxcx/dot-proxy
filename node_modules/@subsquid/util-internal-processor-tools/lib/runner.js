...
class Runner {
    ...
    async run() {
        ...
        await this.processHotBlocks(state).finally(this.chainHeightUpdateLoop(hot));
        return this.exit();
    }
    ...
    async initMetrics(chainHeight, state) {
        ...
        let boundedChainHeight = Math.min(chainHeight, (0, util_internal_range_1.rangeEnd)((0, util_internal_1.last)(this.config.requests).range));
    ...
    async exit() {
        if (isProcessorExitDisabled()) {
            while (true) {
                await (0, util_internal_1.wait)(5000);
            }
        }
    }
