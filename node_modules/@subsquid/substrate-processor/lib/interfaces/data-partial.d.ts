...
export type BlockRequiredFields = 'height' | 'hash' | 'parentHash' | 'specName' | 'specVersion' | 'implName' | 'implVersion';
...
type MakePartial<T, Required extends keyof T, IsArchive extends boolean = false> = Simplify<Pick<T, Required> & {
    [K in keyof T as K extends Required ? never : K]+?: IsArchive extends true ? bigint extends T[K] ? string : T[K] : T[K];
}>;
...
export type ArchiveBlockHeader = MakePartial<Omit<base.BlockHeader, 'height'>, Exclude<BlockRequiredFields, 'height'>, true> & base.WithRuntime & {
    number: number;
};
